...
const package3 = [
  { id: 'retro', src: '/img/eventretro.avif', description: 'Retro' },
  { id: 'funky', src: '/img/funky.avif', description: 'Funky' },
...
const EventShootPackages = [
  { 
    ...
    showAnyDeco: true,
    prices: ['RM49'],
    includes: ['Unlimited Shots (20 mins)', 'All photos edited', '1-3 pax', 'Add on pax - RM10/pax', 'Selfshoot']
  },
  { 
    name: 'Package B', 
    showAnyDeco: true,
    prices: ['RM99'],
    includes: ['Unlimited Shots (20 mins)', 'All photos edited', '1-4 pax', 'Add on pax - RM10/pax', 'Photographer x 1']
  },
  { 
    name: 'Package C', 
    prices: ['RM180'],
    includes: ['Unlimited Shots (40 mins)', 'All photos edited', '1-4 pax', 'Add on pax - RM10/pax', 'Photographer x 1', 'Free film polaroid x 2 (Instax Wide)']
  },
];
...
export default function PortfolioSection() {
  ...
  const package2Images = [
    ...
    const package3Images = [
    { 
      id: 'retro', 
      src: '/img/highimg/eventretro.jpg', 
      lowResSrc: '/img/eventretro.avif',
      alt: 'Retro collection' 
    },
    { 
      id: 'funky', 
  ...
  const lightbox3 = useImageLightbox(package3Images);
  ...
  return (
    <section id="portfolio" className="relative py-20 sm:py-24 overflow-hidden bg-black/50">
          ...
          >
            <div className="mb-16">
              <div className="flex items-center justify-center gap-6 mb-2">
                <div className="h-px w-20 bg-gradient-to-r from-transparent via-white/30 to-white/20"></div>
                <div className="flex flex-col items-center gap-1">
                  <h3 className="text-xs font-light tracking-[0.2em] uppercase text-white/50">Package</h3>
                  <span className="text-3xl font-light tracking-wider text-white">EVENT SHOOT</span>
                </div>
                <div className="h-px w-20 bg-gradient-to-l from-transparent via-white/30 to-white/20"></div>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 justify-items-center mb-8">
              {package3.map((image, index) => (
                <motion.div
                  key={image.id}
                  initial={{ opacity: 0, y: 50 }}
                  whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              <div className="rounded-2xl border border-white/10 bg-gradient-to-b from-[#1A1A1A] to-[#0F0F0F] px-6 py-8 shadow-2xl shadow-black/50">
                <div className="mb-8 text-center">
                  <h3 className="text-2xl font-light tracking-wide text-white mb-2">Package EVENT SHOOT</h3>
                  <div className="mx-auto h-px w-24 bg-gradient-to-r from-transparent via-white/30 to-transparent"></div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                  {EventShootPackages.map((pkg, idx) => (
                    <div key={idx} className="group border border-white/10 rounded-xl p-5 bg-gradient-to-b from-black/40 to-black/20 backdrop-blur-sm transition-all duration-300 hover:border-white/20 hover:shadow-lg hover:shadow-white/5">
                      <h4 className="text-base font-light tracking-wide text-white mb-4 flex items-center flex-wrap">
                        <span>{pkg.name}</span>
                        {pkg.showAnyDeco && <span className="text-xs font-extralight text-white/60 ml-2 italic whitespace-nowrap"></span>}
                      </h4>
                      <div className="flex flex-wrap gap-2 mb-4">
                        {pkg.prices.map((price, priceIdx) => (
                          <span key={priceIdx} className="text-lg font-light tracking-wide text-white bg-white/5 border border-white/10 px-4 py-1.5 rounded-lg backdrop-blur-sm">
      ...
      {/* Lightbox for Package 3 */}
      {lightbox3.current && (
        <ImageLightbox
          src={lightbox3.current.src}
          lowResSrc={lightbox3.current.lowResSrc}
          alt={lightbox3.current.alt}
          isOpen={lightbox3.isOpen}
          onClose={lightbox3.close}
          onNext={lightbox3.next}
          onPrev={lightbox3.prev}
          hasNext={lightbox3.hasNext}
          hasPrev={lightbox3.hasPrev}
          enableZoom={true}
        />
      )}
